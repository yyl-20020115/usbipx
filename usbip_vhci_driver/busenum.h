...
#define Bus_KdPrint(_d_,_l_, _x_) \
            if (!(_l_) || (_d_)->cda.DebugLevel & (_l_)) { \
               DbgPrint ("BusEnum.SYS: "); \
               DbgPrint _x_; \
            }
#define Bus_KdPrint_no_cda(_d_,_l_, _x_) \
...
#define Bus_KdPrint_Cont_no_cda(_d_,_l_, _x_) \
            if (!(_l_) || (_d_)->DebugLevel & (_l_)) { \
               DbgPrint _x_; \
            }
...
#define INITIALIZE_PNP_STATE(_Data_)    \
        (_Data_)->cda.DevicePnPState =  NotStarted;\
        (_Data_)->cda.PreviousPnPState = NotStarted;

#define SET_NEW_PNP_STATE(_Data_, _state_) \
        (_Data_)->cda.PreviousPnPState =  (_Data_)->cda.DevicePnPState;\
        (_Data_)->cda.DevicePnPState = (_state_);

#define RESTORE_PREVIOUS_PNP_STATE(_Data_)   \
        (_Data_)->cda.DevicePnPState =   (_Data_)->cda.PreviousPnPState;\
