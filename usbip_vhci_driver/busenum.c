...
unsigned int tran_usb_status(int linux_status,int in, int type)
{
	switch(linux_status){
		...
		/* I guess it */
		case -EPIPE:
			return (unsigned int)USBD_STATUS_ENDPOINT_HALTED;
		case -EOVERFLOW:
			return (unsigned int)USBD_STATUS_DATA_OVERRUN;
		case -EREMOTEIO:
			return (unsigned int)USBD_STATUS_ERROR_SHORT_TRANSFER;
		default:
			KdPrint(("linux status %d in %d type %d\n", linux_status,
				in, type));
			return (unsigned int)USBD_STATUS_ERROR;
	}
//	return USBD_STATUS_SUCCESS;
}
...
{
	return ((unsigned long long)handle & 0x80)?USBIP_DIR_IN:USBIP_DIR_OUT;
}

static unsigned char INLINE pipe2addr(USBD_PIPE_HANDLE handle)
{
	return (unsigned char)((unsigned long long)handle & 0x7f);
}

static unsigned char INLINE pipe2type(USBD_PIPE_HANDLE handle)
{
	return (unsigned char)(((unsigned long long)handle & 0xff0000)>>16);
}

static unsigned char INLINE pipe2interval(USBD_PIPE_HANDLE handle)
{
	return (unsigned char)(((unsigned long long)handle & 0xff00)>>8);
}
    ...
    )
{
    ...
    if (fdoData->cda.DevicePnPState == Deleted){
		...
		unsigned int devid)
{
	...
	*copied = 0;
	...
	KdPrint(("PipeHandle %016xull\n", (unsigned long long)req->PipeHandle));
...

{
    ...
    result = InterlockedIncrement((volatile LONG*)&FdoData->OutstandingIO);
...
--*/
{
    ...
    result = InterlockedDecrement((volatile LONG*)&FdoData->OutstandingIO);
    ...
    if (result == 0) {
        ...
        ASSERT(FdoData->cda.DevicePnPState == Deleted);
