...
--*/
{
    ...
    if (commonData->IsFDO) {
        ...
        Bus_KdPrint_no_cda (commonData, BUS_DBG_POWER_TRACE,
            ("FDO %s IRP:0x%p %s %s\n",
...

{
    ...
    if (Data->cda.DevicePnPState == NotStarted) {
        PoStartNextPowerIrp(Irp);
    ...
    case IRP_MN_SET_POWER:
        ...
        switch (powerType) {
            case DevicePowerState:
                PoSetPowerState (PdoData->cda.Self, powerType, powerState);
                PdoData->cda.DevicePowerState = powerState.DeviceState;
                status = STATUS_SUCCESS;
                break;

            case SystemPowerState:
                PdoData->cda.SystemPowerState = powerState.SystemState;
                status = STATUS_SUCCESS;
                break;
