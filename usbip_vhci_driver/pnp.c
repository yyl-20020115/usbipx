...
--*/
{
    ...
    deviceData->cda.DebugLevel = BusEnumDebugLevel;

    deviceData->cda.IsFDO = TRUE;

    deviceData->cda.Self = deviceObject;
    ...
    deviceData->cda.DevicePowerState = PowerDeviceUnspecified;
    deviceData->cda.SystemPowerState = PowerSystemWorking;
...
--*/
{
    ...
    if (commonData->IsFDO) {
        Bus_KdPrint_no_cda(commonData, BUS_DBG_PNP_TRACE,
                      ("FDO %s IRP:0x%p\n",
...
--*/
{
    ...
    case IRP_MN_CANCEL_STOP_DEVICE:
        ...
        if (StopPending == DeviceData->cda.DevicePnPState)
        {
            //
            // We did receive a query-stop, so restore.
            //
            RESTORE_PREVIOUS_PNP_STATE(DeviceData);
            ASSERT(DeviceData->cda.DevicePnPState == Started);
        }
    ...
    case IRP_MN_CANCEL_REMOVE_DEVICE:
        ...
        if (RemovePending == DeviceData->cda.DevicePnPState)
        {
            //
    ...
    case IRP_MN_REMOVE_DEVICE:
        ...
        if (DeviceData->cda.DevicePnPState != SurpriseRemovePending)
        {
            Bus_RemoveFdo(DeviceData);
        }
        ...
        for(entry = listHead->Flink,nextEntry = entry->Flink;
            ...
            pdoData = CONTAINING_RECORD (entry, PDO_DEVICE_DATA, Link);
            RemoveEntryList (&pdoData->Link);
            if (SurpriseRemovePending == pdoData->cda.DevicePnPState)
            {
                //
            ...
            DeviceData->NumPDOs--;
            Bus_DestroyPdo (pdoData->cda.Self, pdoData);
        }
    ...
    case IRP_MN_QUERY_DEVICE_RELATIONS:
        ...
        for (entry = DeviceData->ListOfPDOs.Flink;
            ...
            pdoData = CONTAINING_RECORD (entry, PDO_DEVICE_DATA, Link);
            if (pdoData->Present) {
                relations->Objects[prevcount] = pdoData->cda.Self;
                ObReferenceObject (pdoData->cda.Self);
                prevcount++;
            } else {
                pdoData->ReportedMissing = TRUE;
            }
...
--*/
{
    ...
    FdoData->cda.DevicePowerState = PowerDeviceD0;
    powerState.DeviceState = PowerDeviceD0;
    PoSetPowerState ( FdoData->cda.Self, DevicePowerState, powerState );
    ...
    )
{
    ...
    // Initialize the rest
    //
    pdodata->cda.IsFDO = FALSE;
    pdodata->cda.Self =  pdo;
    pdodata->cda.DebugLevel = BusEnumDebugLevel;

    pdodata->ParentFdo = fdodata->cda.Self;
    ...
    pdodata->cda.DevicePowerState = PowerDeviceD3;
    pdodata->cda.SystemPowerState = PowerSystemWorking;
		...
		PFILE_OBJECT fo)
{
    ...
    for (entry = fdodata->ListOfPDOs.Flink;
        ...
        if (plugin->addr == pdodata->SerialNo &&
            pdodata->cda.DevicePnPState != SurpriseRemovePending)
        {
	    ExReleaseFastMutex (&fdodata->Mutex);
	    return STATUS_INVALID_PARAMETER;
        }
    ...
    status = IoCreateDeviceSecure(fdodata->cda.Self->DriverObject,
		...
		PFDO_DEVICE_DATA  fdodata, ULONG *info)
{
    ...
    for (entry = fdodata->ListOfPDOs.Flink;
		...
		if(st->u.max_used_port < (int)pdodata->SerialNo)
			st->u.max_used_port = (char)(int)pdodata->SerialNo;
    ...
    if (found) {
		...
		for (entry = fdodata->ListOfPDOs.Flink;
			...
			if( pdodata->Present ==FALSE){
				...
				IoInvalidateDeviceState(pdodata->cda.Self);
			}
		}
		ExReleaseFastMutex (&fdodata->Mutex);
    ...
    for (entry = FdoData->ListOfPDOs.Flink;
        ...
        if (ejectAll || Eject->SerialNo == pdoData->SerialNo) {
            Bus_KdPrint (FdoData, BUS_DBG_IOCTL_INFO,
                          ("Ejected %d\n", pdoData->SerialNo));
            found = TRUE;
            IoRequestDeviceEject(pdoData->cda.Self);
            if (!ejectAll) {
                break;
            }
        }
